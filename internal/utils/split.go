...
func Split(source []int, batchSize int) ([][]int, error) {
	...
	// Слайс это структура которая имеет указатель на выделенный участок памяти с массивом.
	// При передачи в функции слайс копируется, но вместе с указателем на тот же участок памяти,
	// из за этого при изменении данных переданого слайса меняется данные и в исходном.
	...
	// Колличество шагов (батчей), округляем в большую сторону
